...
The system is designed with strict prompt engineering to prevent hallucinations and includes a resilient multi-model fallback mechanism to ensure high availability.

## Features
- **Strict JSON Extraction:** Forces the LLM to output a clean, parseable JSON schema.
- **Self-Correcting Auditor:** Includes a secondary LLM validation step (`get_validation_prompt`) that checks the initial extraction against the original text, preventing hallucinations and correcting formatting issues.
...
- **Dual Interfaces:** Can be run as a lightweight CLI tool (`main.py`) or as a modern Web UI with a FastAPI backend (`api.py` & `app.py`).
